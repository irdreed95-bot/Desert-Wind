...
export function ProductCard({ product }: { product: Product }) {
  ...
  return (
    <div className="flex-shrink-0 w-[220px] md:w-[260px] bg-card border border-border rounded-xl overflow-hidden group hover:border-primary/50 transition-colors flex flex-col h-full">
      {/* Image Area */}
      <div className="h-[170px] bg-background relative overflow-hidden flex items-center justify-center p-3">
        {product.badge && (
        ...
        <div className="mt-auto pt-3">
          ...
          <div className="flex items-center gap-2 mb-3 text-xs font-medium">
            {product.stockStatus === 'in_stock' && (
              <><span className="w-2 h-2 rounded-full bg-emerald-500 flex-shrink-0"></span><span className="text-emerald-500">{product.stockLabel || 'متوفر'}</span></>
            )}
            {product.stockStatus === 'low_stock' && (
              <><span className="w-2 h-2 rounded-full bg-orange-500 flex-shrink-0"></span><span className="text-orange-500">{product.stockLabel}</span></>
            )}
            {product.stockStatus === 'out_of_stock' && (
              <><span className="w-2 h-2 rounded-full bg-red-500 flex-shrink-0"></span><span className="text-red-500">{product.stockLabel || 'نفذ من المخزون'}</span></>
            )}
          </div>
